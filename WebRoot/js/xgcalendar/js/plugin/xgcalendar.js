...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        var def = {
            ...
            jiabanItems: [], //日程数据，可通过此参数设置初始化数据
            zhibanItems: [], //日程数据，可通过此参数设置初始化数据
            ...
            quickAddHandler: true, //快速添加的拦截函数，该参数设置后quickAddUrl参数的设置将被忽略
...
//                    for(var a=0;a<option.jiabanItems.length;a++){
//                           for(var b=0;b<option.jiabanItems[a].events.length;b++){
//                               jbevents.push(option.jiabanItems[a].events[b]);
//                           }
//                    }
//                    
//                     for(var a=0;a<option.zhibanItems.length;a++){
//                           for(var b=0;b<option.zhibanItems[a].events.length;b++){
//                               zbevents.push(option.zhibanItems[a].events[b]);
//                           }
//                    }
                    ...
                    var jiabanarr=[];
                    var jiabantemp="";
                    for(var n=0;n<jbevents.length;n++){
                         var start=jbevents[n][2];
                         var jiabanriqi=dateFormat.call(start, "yyyy-MM-dd");
                         var nowday=dateFormat.call(day, "yyyy-MM-dd");
                         if(nowday==jiabanriqi){
                            jiabanarr=jbevents[n];
                            bo=true;
                            break;
                        } 
                    }
                     ...
                     if(bo){
                        var pos=getCenterPositon2();
                       // alert("result:"+JSONstringify(jiabanarr));
                        jiabantemp = "<div aligh='rightprompt' id='right' style='float:right;color:#8c66d9;font-weight:bold;font-size:15px;' ><img src='"+context+"/js/xgcalendar/image/jia.png' width='16px' height='16px' onclick='jiabanaciton("+JSON.stringify(jiabanarr)+","+pos.left+","+pos.top+");' class='monthdayshow'/></div>";
                     }
                   
                     var bo1=false;
                     var zhibantemp="";
                     var zhibanarr=[];
                     for(var n=0;n<zbevents.length;n++){
                         var start=zbevents[n][1];
                         var zhibanriqi=dateFormat.call(start, "yyyy-MM-dd");
                         var nowday=dateFormat.call(day, "yyyy-MM-dd");
                         if(nowday==zhibanriqi){
                            zhibanarr=zbevents[n];
                            bo1=true;
                            break;
                        } 
                    }
                    ...
                    if(bo1){
                      var pos=getCenterPositon2();
                     // alert(JSON.stringify(zhibanarr));
                    zhibantemp = "<div aligh='rightprompt' id='right1' style='float:right;color:#8c66d9;font-weight:bold;font-size:15px;' ><img src='"+context+"/js/xgcalendar/image/zhi.png' width='16px' height='16px' onclick='zhibanaciton("+JSON.stringify(zhibanarr)+","+pos.left+","+pos.top+");' class='monthdayshow'/></div>";
                    }
                     
                     titletemp = "<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a>"+jiabantemp+zhibantemp+"</td>";
                     
                    //alert((day.getMonth()+1) +"  "+day.getDate());
                   //	titletemp="<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a><div aligh='rightprompt'  id='jiaban' style='float:right;'><img src='"+context+"/js/xgcalendar/image/jia.png' width='16px' height='16px'/></div><div  aligh='rightprompt' id='zhiban' style='float:right;margin-right:3px'><img src='"+context+"/js/xgcalendar/image/zhi.png' width='16px' height='16px' /></div></td>";
                    //	titletemp="<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a></td>";
                 
                  // titletemp = "<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a><div aligh='rightprompt' id='right' style='float:right;color:#8c66d9;font-weight:bold;font-size:15px;' ><img src='"+context+"/js/xgcalendar/image/jia.png' width='16px' height='16px' onclick='alert(111111111);' class='monthdayshow'/></div></td>";
               
                    o.abbr = dateFormat.call(day, i18n.xgcalendar.dateformat.fulldayvalue);
                    //alert(JSON.stringify(formatevents[j][i])); 
                    htb.push(Tp(titletemp, o));
                }
                htb.push("</tr>");
                var sfirstday = C[j * 7];
                //alert(JSON.stringify(formatevents[j]));
        ...
        function BuildMonthRow(htr, events, dMax, sc, day) {
            ...
            for (var j = 0; j < l; j++) {
                var ec = events[j] ? events[j].length : 0;//每天的日志数
                y[j] += ec;
                for (var k = 0; k < ec; k++) {
                    var e = events[j][k];
                    if (e && e.colSpan > 1) {
          ...
          if (option.isloading) {
           ...
           // alert(startdate+" "+enddate);
            ...
            if (option.url && option.url != "") {
				...
				{ name:"vstart",value:  dateFormat.call(startdate, i18n.xgcalendar.dateformat.fulldayvalue) },   
				{ name:"vend",value:  dateFormat.call(enddate, i18n.xgcalendar.dateformat.fulldayvalue) }
                ];
                if (option.extParam) {
                ...
                $.ajax({
                    ...
                    success://function(data) {
						...
						eval("dataarr = "+datastr+";");		
                         ...
                         data=dataarr[0];
                        // option.jiabanItems.push(dataarr[1]);
                        // option.zhibanItems.push(dataarr[2]);
        ...
        function parseED(data) {
            if (data.length > 6) {
                var e = [];
                e.push(data[0], data[1], new Date(data[2]), new Date(data[3]), parseInt(data[4],10), parseInt(data[5],10), parseInt(data[6],10), data[7] != undefined ? parseInt(data[7],10) : -1, data[8] != undefined ? parseInt(data[8],10) : 0, data[9], data[10],data[11]);
                return e;
            }
            return null;
        ...
        function getCenterPositon(){
            var maxLeft = document.documentElement.clientWidth;
            var maxTop = document.documentElement.clientHeight;
            
            var left=maxLeft/2-300;
            var top=maxTop/2-220;
              return { left: left, top: top, hide: false };
        }
        function getCenterPositon2(){
            var maxLeft = document.documentElement.clientWidth;
            var maxTop = document.documentElement.clientHeight;
            
            var left=maxLeft/2-150;
            var top=maxTop/2-120;
              return { left: left, top: top, hide: false };
        }
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                  ...
                  window.open(context+"/manager/activity/showView.do?ID="+data[0]);
                  ...
                  return;
                   ...
                   var rz_update="";
                    ...
                    var csbuddle = '<div id="bbit-cs-buddle" style="z-index: 990; width: 850px;visibility:hidden;" class="bubble"><table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div><td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div>  <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root"><table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><div id="bbit-cs-what"'
                    	+ 'class="textbox-fill-div" style="font-size:22px;font-family:黑体"></div></div></div></td></tr><tr><td class=cb-value><div id="bbit-cs-buddle-timeshow" style="text-align: right;"></div></td></tr></tbody></table><div class="bbit-cs-split"><div id="bbit-cs-content" style="max-height:450px;height:450px;overflow-y:auto;line-height:25px;padding-left:5px;padding-right:5px;" /><div class="bbit-cs-split" ><div style="text-align:right"><input id="bbit-cs-id" type="hidden" value=""/> <span id="bbit-cs-delete" class="lk">'
                    	+ rz_delete + '</span> &nbsp; <SPAN id="bbit-cs-editLink" class="lk">'
                    	+ rz_update + ' </SPAN></div></div></div></div></div><tr><td><div id="bl1" class="bubble-corner"><div class="bubble-sprite bubble-bl"></div></div><td><div class="bubble-bottom"></div><td><div id="br1" class="bubble-corner"><div class="bubble-sprite bubble-br"></div></div></tr></tbody></table><div id="bubbleClose2" class="bubble-closebutton"></div><div id="prong1" class="prong"><div class=bubble-sprite></div></div></div>';
...
//                        });
                    ...
                    $("#bbit-cs-content").html(decodeURIComponent(data[11]));
                    //alert(decodeURI(data[11]));
                    bud.data("cdata", data);
                    
                    
                    bud.css({ "visibility": "visible", left: 100, top:50 });

                    $(document).one("click", function(ee) {
                         if(ee.which==0){
                             $("#bbit-cs-buddle").css("visibility", "hidden");
                         } 
        ...
        function dayupdate(data, start, end) {
         ...
         function compareDate(d1,d2){
    		var dd1=new Date(d1.replace("-", "/").replace("-", "/"));  
    		var dd2=new Date(d2.replace("-", "/").replace("-", "/"));  
    		if(dd2>dd1){
        		return true;  
    		}
   			    return false;  
        ...
        function quickadd(start, end, isallday, pos) {
           realsedragevent();
             ...
             if ((!option.quickAddHandler && option.quickAddUrl == "") || option.readonly) {
              ...
              var today=dateFormat.call(new Date(), "yyyy-MM-dd") ;
              var startdate=dateFormat.call(new Date(start), "yyyy-MM-dd") ;
             var bo=compareDate(today,startdate);
             if(bo){//如果开始日期大于今天
                 realsedragevent();
                 if(adminid==""){
                  alert("不在添加日志日期范围");
               ...
               var bo1=compareDate(startdate,adddays);
               if(bo1){
                  realsedragevent();
                if(adminid==""){
                  alert("不在添加日志日期范围");
            ...
            if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
             			 var param ={ "start": dateFormat.call(start, "yyyy-MM-dd") ,"end":dateFormat.call(end, "yyyy-MM-dd"),"isallday":isallday,"pos":pos,"dateshow":dateshow,"zone":zone};
