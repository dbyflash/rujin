...
$(function() {
	...
	$("#cost_add_btn").click(function(){
		$("#cost_edit_box").show().dialog({
			...
			height:250,
			modal:true,
			href:'${ctx }/manager/cost/create.do',
			title:'添加成本费用',
			buttons: [{
					text:'确定',
					iconCls:'icon-ok',
	...
	$("#cost_edit_btn").click(function(){
		...
		if(row){
			var url = "${ctx }/manager/cost/edit.do?id="+row.id;
			$("#cost_edit_box").show().dialog({
				width:600,
				height:250,
				modal:true,
	...
	$("#doSearch").click(function(){
	   $("#cost_list_box").datagrid({
		  url : "${ctx }/manager/cost/list.do?s_name="+encodeURI($("#s_name").val())+"&s_cnumber="+encodeURI($("#s_cnumber").val())+"&s_begin="+$("#s_begin").val()+"&s_end="+$("#s_end").val()
		  	   });
...
<div region="center" border="false" >
	  ...
	  url:'${ctx }/manager/cost/list.do?type=0'" >
		                  <thead>
			               <tr>
				             <th data-options="field:'id',hidden:true">ID</th>
				             <th data-options="field:'name',width:120,halign:'center',align:'center'">费用名称</th>
				             <th data-options="field:'style',width:120,halign:'center',align:'center'">计量单位</th>
