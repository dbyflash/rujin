...
<script type="text/javascript">
	...
	function search_collector(nodeid,type){
	    ...
	    $("#search_provider_list_box").datagrid({
		  url : "${ctx }/manager/provider/list.do?s_name="+encodeURI($("#s_name").val())
	   });
	}
	
	function savesearch_provider(idinput,nameinput){
	   var row = $("#search_provider_list_box").datagrid("getSelected"); 
	   if(row==null){
	       rightBottom("请选择供应商");
	   }
      $("#"+idinput).val(row.id);
...
<div data-options="region:'center'" fit="true" border="false">
    <table id="search_provider_list_box"  class="easyui-datagrid"  
		                  ...
		                  <thead>
			               <tr>
			                <th data-options="field:'ck',checkbox:true"></th>
				             <th data-options="field:'id',hidden:true">供应商ID</th>
				             <th data-options="field:'name',width:250,halign:'center',align:'left'">供应商名称</th>
			              </tr>
		                </thead>
	  </table>
</div>
