...
public class SendController extends PerssoftController<Send> {

	
	@Override
	public void index() {
		render("/WEB-INF/jsp/manager/send/sendList.jsp");
	}
	
	@Override
	public void create() {
		super.create();
		render("/WEB-INF/jsp/manager/send/sendForm.jsp");
	}
	...
	public void list() {
		...
		PageData<Send> list=Send.dao.pageList(map);
		renderJson(list);
	}
	@Override
	public void save() {
		...
		if(!check){
			json = new Json(Json.STAE_CODE_ERROR, "发料单已存在！");
			renderJson(json);
			return;
		}
		try {

			Send object = getModel(modelClass);
	...
	public boolean editCheck(){
		...
		Map map=getParaMap();
		map.put("id", id);
		Send bean=Send.dao.getByNumberId(map,number);
		if(bean==null){
			return true;
		}else{
			return false;
