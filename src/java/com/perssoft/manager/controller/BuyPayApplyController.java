...
public class BuyPayApplyController extends PerssoftController<BuyPayApply> {

	
	@Override
	public void index() {
		render("/WEB-INF/jsp/manager/buypayapply/buypayapplyList.jsp");
	}
	
	@Override
	public void create() {
		super.create();
		render("/WEB-INF/jsp/manager/buypayapply/buypayapplyForm.jsp");
	}
	...
	@Override
	public void list() {
		PageData<BuyPayApply> list=BuyPayApply.dao.pageList(getParaMap());
		renderJson(list);
	}
	@Override
	public void save() {
		...
		if(!check){
			json = new Json(Json.STAE_CODE_ERROR, "付款申请已存在！");
			renderJson(json);
			return;
		}
		try {

			BuyPayApply object = getModel(modelClass);
	...
	@Override
	protected boolean doAfterSave(BuyPayApply object) {
		//将合同编号写入对应的坯布流转
	...
	public boolean editCheck(){
		...
		Map map=getParaMap();
		map.put("id", id);
		BuyPayApply bean=BuyPayApply.dao.getByNumberId(map,number);
		if(bean==null){
			return true;
		}else{
			return false;
